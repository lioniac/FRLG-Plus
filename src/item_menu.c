...
//register LR
static void Task_ItemMenuAction_ButtonSEL(u8 taskId);
...
static void Task_ItemMenuAction_Unregister(u8 taskId);
...
static const struct MenuAction sItemMenuContextActions[] = {
    ...
    [ITEMMENUACTION_DESELECT] =     {gOtherText_Deselect, {.void_u8 = Task_ItemMenuAction_Unregister}},
    ...
    [ITEMMENUACTION_SEL_BUTTON] =   {sMenuText_SEL, Task_ItemMenuAction_ButtonSEL},
...
static const u8 sButtonSELGfx[] = INCBIN_U8("graphics/interface/button_SEL.4bpp");
static const u8 sButtonLGfx[]   = INCBIN_U8("graphics/interface/button_L.4bpp");
static const u8 sButtonRGfx[]   = INCBIN_U8("graphics/interface/button_R.4bpp");
...
static void BagListMenuItemPrintFunc(u8 windowId, s32 itemId, u8 y)
{
    ...
    if (itemId != -2 && sBagMenuDisplay->nItems[gBagMenuState.pocket] != itemId)
    {
        ...
        {
            if (gSaveBlock1Ptr->registeredItemSEL != ITEM_NONE && gSaveBlock1Ptr->registeredItemSEL == bagItemId)
                BlitBitmapToWindow(windowId, sButtonSELGfx, 0x70, y, 0x18, 0x10);

            if (gSaveBlock1Ptr->registeredItemL   != ITEM_NONE && gSaveBlock1Ptr->registeredItemL   == bagItemId)
                BlitBitmapToWindow(windowId, sButtonLGfx,   0x70, y, 0x18, 0x10);

            if (gSaveBlock1Ptr->registeredItemR   != ITEM_NONE && gSaveBlock1Ptr->registeredItemR   == bagItemId)
                BlitBitmapToWindow(windowId, sButtonRGfx,   0x70, y, 0x18, 0x10);
        }
    }
}
        ...
        else
        {
            ...
            case 2:
                ...
                if (sRegisterSubMenu == FALSE)
                {
                    ...
                    if (gSaveBlock1Ptr->registeredItemSEL == gSpecialVar_ItemId)
...
bool8 UseRegisteredKeyItemOnField(u8 button)
{
    ...
    {
        case 0:
            registeredItem = gSaveBlock1Ptr->registeredItemSEL;
            break;
        case 1:
    ...
    if (registeredItem != ITEM_NONE)
    {
        ...
        else
        {
            ...
            {
            case 0:
                gSaveBlock1Ptr->registeredItemSEL = ITEM_NONE;
                break;
            case 1:
...
static void BackUpPlayerBag(void)
{
    ...
    sBackupPlayerBag->registeredItemSEL = gSaveBlock1Ptr->registeredItemSEL;
...
static void RestorePlayerBag(void)
{
    ...
    gSaveBlock1Ptr->registeredItemSEL = sBackupPlayerBag->registeredItemSEL;
...
static void Task_Bag_TeachyTvRegister(u8 taskId)
{
    ...
    if (!gPaletteFade.active && Task_BButtonInterruptTeachyTv(taskId) != TRUE)
    {
        ...
        case 510:
            PlaySE(SE_SELECT);
            gSaveBlock1Ptr->registeredItemSEL = gSpecialVar_ItemId;
            HideBagWindow(10);
            HideBagWindow(6);
            PutWindowTilemap(0);
            PutWindowTilemap(1);
...
static void Task_ItemMenuAction_ButtonSEL(u8 taskId)
{
    ...
    gSaveBlock1Ptr->registeredItemSEL = itemId;
